...
QString QHotkey::shortcut() const
{
	if(_keyCode == Qt::Key_unknown)
        return QKeySequence().toString();
	else
        return QKeySequence(_keyCode | _modifiers).toString();
}

unsigned long QHotkey::keyCode() const
{
    return (unsigned long)_keyCode;
}

unsigned long QHotkey::modifiers() const
{
    return (unsigned long)_modifiers;
}
...
bool QHotkey::setKeySequenceShortcut(const QKeySequence &shortcut, bool autoRegister)
{
    ...
    return setKeyShortcut(Qt::Key(shortcut[0] & ~Qt::KeyboardModifierMask),
			Qt::KeyboardModifiers(shortcut[0] & Qt::KeyboardModifierMask),
			autoRegister);
}

bool QHotkey::setKeyShortcut(Qt::Key keyCode, Qt::KeyboardModifiers modifiers, bool autoRegister)
{
	if(_registered) {
		if(autoRegister) {
